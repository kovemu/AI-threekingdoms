...
pub fn manifest() -> Result<Manifest, String> {
    serde_json::from_str(include_str!("../../src/ai/model-manifest.json"))
...
pub fn select_profile(h: &Hardware) -> &'static str {
    ...
    if h.ram_gb >= 15.0 && h.vram_gb >= 7.5 && h.free_vram_gb >= 6.2 {
        "default"
    } else {
        "low"
    }
...
pub async fn detect_hardware() -> Hardware {
    ...
    let mut h = Hardware {
        ram_gb: system.total_memory() as f64 / 1073741824.0,
        vram_gb: 0.0,
        free_vram_gb: 0.0,
    };
    ...
    if let Ok(Ok(out)) = tokio::time::timeout(Duration::from_secs(4), cmd.output()).await {
        ...
        if let Some(line) = String::from_utf8_lossy(&out.stdout).lines().next() {
            let values: Vec<f64> = line
                ...
                .collect();
            if values.len() == 2 {
                h.vram_gb = values[0] / 1024.0;
                h.free_vram_gb = values[1] / 1024.0;
            }
        }
    }
    h
...
async fn verified(path: &Path, hash: &str, bytes: u64) -> Result<bool, String> {
    ...
    tokio::task::spawn_blocking(move || verify_file(&p, &h, bytes))
        .await
        .map_err(|e| e.to_string())?
}
pub async fn acquire(
    ...
    if !url.starts_with("https://")
        ...
        || filename.contains(['/', '\\'])
    {
        return Err("다운로드 manifest 오류".into());
    }
    ...
    if verified(&dest, hash, bytes).await? {
        return Ok(dest);
    }
    if dest.exists() {
    ...
    if verified(&part, hash, bytes).await? {
    ...
    if response.status() == reqwest::StatusCode::PARTIAL_CONTENT {
        ...
        {
            return Err("서버의 이어받기 범위 오류".into());
        }
    } else {
        offset = 0;
    }
    ...
    while let Some(chunk) = stream.next().await {
        ...
        if downloaded > bytes {
            ...
            return Err("예상 파일 크기 초과".into());
        }
        file.write_all(&chunk)
            .await
            .map_err(|e| format!("저장 공간을 확인해 주세요: {e}"))?;
        if last.elapsed() >= Duration::from_millis(200) {
            let mut s = Status::new("downloading", &format!("자동 다운로드: {filename}"));
    ...
    progress(Status::new(
        "verifying",
        &format!("SHA-256 검사: {filename}"),
    ));
    if !verified(&part, hash, bytes).await? {
        let _ = tokio::fs::remove_file(&part).await;
        return Err("파일 검증 실패. 다시 시도해 주세요.".into());
    }
    tokio::fs::rename(&part, &dest)
        .await
        .map_err(|e| e.to_string())?;
...
pub fn extract_runtime(archive: &Path, dir: &Path, executable: &str) -> Result<PathBuf, String> {
    ...
    for i in 0..zip.len() {
        ...
        if entry.unix_mode().is_some_and(|m| m & 0o170000 == 0o120000) {
            return Err("런타임 심볼릭 링크 거부".into());
        }
        total += entry.size();
        if total > 500_000_000 {
            return Err("런타임 압축 크기 초과".into());
        }
        ...
        if path.file_name().is_some_and(|n| n == executable) {
            found = Some(path);
        }
    }
    found.ok_or("런타임 실행 파일이 없습니다.".into())
}
pub fn lock_directory(root: &Path) -> Result<std::fs::File, String> {
    ...
    file.try_lock().map_err(|_| {
        "게임이 이미 실행 중입니다. 기존 게임 창을 사용하거나 닫은 뒤 다시 시도해 주세요."
            .to_string()
    })?;
    Ok(file)
}
...
mod tests {
    use super::*;
    #[test]
    fn second_instance_cannot_share_downloads_until_first_exits() {
